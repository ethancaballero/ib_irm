...
class irm_model:
    ...
    def fit(self,data_tuple_list):
        ...
        def loss_0(model, x,e,y,w):
            y_ =model(x)
            var_penalty_all = tf.reduce_mean(tf.math.reduce_variance(y_, axis=0, keepdims=False))
            loss_object = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
            return var_penalty_all, loss_object(y_true=y, y_pred=y_)
        
        # sum of cross entropy loss and penalty 
        def loss_total(model,x,e,y,w,gamma, n_e):
            var_penalty_all, loss0 = loss_0(model,x,e,y,w)
            loss_penalty = 0.0
            #var_penalty_envs = 0.0
            for k in range(n_e):
                loss_penalty += gamma*grad_norm_n(model,x,e,y,w,k)

            loss_penalty += gamma * var_penalty_all
